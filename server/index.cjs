...
const server = http.createServer(async (req, res) => {
  ...
  // ttyd proxy - no auth check here, ttyd has its own auth
  ...
  if (urlPath === '/api/correctEnglish' && req.method === 'POST') {
    ...
    try {
      ...
      const hfBody = JSON.stringify({ inputs: `Correct this English text: ${text}`, parameters: { max_length: 200, min_length: 10 } });
        ...
        hfRes.on('end', () => {
          try {
            if (hfRes.statusCode !== 200) {
              let corrected = text.replace(/\br\s+you\b/gi, 'are you').replace(/\bhow old a you\b/gi, 'how old are you').replace(/\bu\b/gi, 'you').replace(/\br\b/gi, 'are').replace(/^([a-z])/, (m) => m.toUpperCase());
              return json(res, { success: true, correctedText: corrected.trim() });
            }
            const result = JSON.parse(data);
            let correctedText = (result[0]?.summary_text || result[0]?.generated_text || text).replace(/^Correct this English text:\s*/i, '').replace(/^["']|["']$/g, '').trim();
            json(res, { success: true, correctedText });
          } catch (e) {
            let corrected = text.replace(/\br\s+you\b/gi, 'are you').replace(/\bu\b/gi, 'you').replace(/\br\b/gi, 'are').replace(/^([a-z])/, (m) => m.toUpperCase());
